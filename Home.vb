...
Public Class Home
    ...
    'Parametres
    Public SMA, ECC, INC, RAAN, AOP, MA, TA, EA, ETFP, AOL, LAT, LONGI, APA, PEA, APR, PER, ALT, MALT, APV, PEV, VEL, BSTAR, _
    ...
    Public XC, YC, ZC, VXC, VYC, VZC, SMAC, ECCC, INCC, RAANC, AOPC, MALTC, XSC, YSC, ZSC, LNGC As Double
    Public APER, DPER, KPER, LANO, LAN, MM, NP, AP, DL, N0, DN As Double
    ...
    ' Time Localization
    Public DS As String = System.Globalization.DateTimeFormatInfo.CurrentInfo.DateSeparator
    Public TS As String = System.Globalization.DateTimeFormatInfo.CurrentInfo.TimeSeparator
    ...
    'Version du logiciel
    Public TLEAVersion As String = "1.14"
    ...
    Private Sub Sat_ListBox_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Sat_ListBox.SelectedIndexChanged
        ...
        If TLE_ListBox.SelectedItem = "favorites.txt" Then 'dans le cas de fichier TLE "FAVORIS"
            ...
            Catch Ex As Exception
                FileClose(1)
                MessageBox.Show(FavAdress & " seems to be corrupted :" & vbCrLf & "Please read User Manual for help." & vbCrLf & vbCrLf & Ex.Message, "TLE ANALYSER - Error")
            End Try
        ...
        Catch ex As Exception
            MessageBox.Show(TLEFILE & " seems to be corrupted :" & vbCrLf & "Please use TLE Updater." & vbCrLf & vbCrLf & ex.Message, "TLE ANALYSER - Error")
        End Try
    ...
    Sub LoadTLE()
        ...
        EPOCH0 = (1721424.5 - Int((EpoquYear - 1) / 100) + Int((EpoquYear - 1) / 400) + Int(365.25 * (EpoquYear - 1)) + (EpoquDay - DELTAD))
        TLEPOCH = EPOCH
        ...
        TLEEpochBox.Text = MJDtoGreg2(TLEPOCH)
    ...
    Sub ElementSecondaires()
        ...
        AOL = (AOP + Maths.RAD2DEG * (TA)) Mod 360

        'Elapsed Time Form Periapsis
        ETFP = (Maths.DEG2RAD * (MA) / (Sqrt(Mu / SMA ^ 3))) / 60
        ETFP_Label.Text = "min"

        'Precession/Variations (au 1er ordre: J2)
        'DW deg/day
        NP = Maths.RAD2DEG * -((3 * Sqrt(Mu / SMA ^ 3) * J2 * EarthEquRad ^ 2 * Cos(Maths.DEG2RAD * (INC)) / (2 * (1 - ECC ^ 2) ^ 2 * SMA ^ 2))) * 86400

        'Dw deg/day
        AP = Maths.RAD2DEG * -((3 * Sqrt(Mu / SMA ^ 3) * J2 * EarthEquRad ^ 2 * (1 - 5 * Cos(Maths.DEG2RAD * (INC)) ^ 2)) / (4 * (1 - ECC ^ 2) ^ 2 * SMA ^ 2)) * 86400
        'Dw/n rad/s
        AopSecVar = (3 / (4 * (1 - ECC ^ 2) ^ 2)) * J2 * (EarthEquRad / SMA) ^ 2 * (5 * Cos(Maths.DEG2RAD * (INC)) ^ 2 - 1)

        'Dn deg/day
        DN = Maths.RAD2DEG * ((3 * Sqrt(Mu / SMA ^ 3) * J2 * EarthEquRad ^ 2 * (3 * Cos(Maths.DEG2RAD * (INC)) ^ 2 - 1)) / (4 * (1 - ECC ^ 2) ^ (3 / 2) * SMA ^ 2)) * 86400
        'Dn/n rad/s
        MeanMouvSecVar = (3 / (4 * (1 - ECC ^ 2) ^ (3 / 2))) * J2 * (EarthEquRad / SMA) ^ 2 * (3 * Cos(Maths.DEG2RAD * (INC)) ^ 2 - 1)

        'Periodes
        KPER = 2 * PI * Sqrt(SMA ^ 3 / Mu) / 60 'min
        APER = (1 - MeanMouvSecVar) * KPER
        ...
        If NP < 0.975 OrElse NP > 0.995 Then
            ...
            FindNT0 = False
            For Spec = 0.01 To 0.05 Step 0.000001
    ...
    Sub OpenTleFile(ByVal Fichier As String)
        ...
        Else 'dans le cas des fichiers TLE standard
            ...
            Catch Ex As Exception
                FileClose(1)
                MessageBox.Show(Fichier & " seems to be corrupted :" & vbCrLf & "&a TLE Updater." & vbCrLf & vbCrLf & Ex.Message, "TLE ANALYSER - Error")
            End Try
        End If
...
Err:
    ...
    Private Sub EpochFormat_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles EpochFormat.SelectedIndexChanged
        ...
        If EpochFormat.SelectedIndex = 1 Then

            EPOCHBox.Text = MJDtoGreg2(EPOCH)
            TLEEpochBox.Text = MJDtoGreg2(TLEPOCH)

        ElseIf EpochFormat.SelectedIndex = 0 Then

            EPOCHBox.Text = EPOCH
            TLEEpochBox.Text = TLEPOCH
